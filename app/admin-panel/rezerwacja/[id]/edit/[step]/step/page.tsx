...
export default function EditReservationPage() {
  ...
  useEffect(() => {
    if (currentStep < 1 || currentStep > 4) {
      router.push(`/admin-panel/rezerwacja/${reservationNumber}/edit/1/step`);
    }
  }, [currentStep, reservationNumber, router]);
  ...
  const handleStepChange = (step: number) => {
    router.push(`/admin-panel/rezerwacja/${reservationNumber}/edit/${step}/step`);
  };
          ...
          <div className="flex-1 overflow-auto" style={{ maxHeight: 'calc(100vh - 250px)' }}>
            {currentStep === 1 && reservation && (
              <EditReservationStep1
                data={{
                  parents_data: (reservation.parents_data || []).map((parent: any, index: number) => ({
