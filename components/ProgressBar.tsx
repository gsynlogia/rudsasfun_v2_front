...
export default function ProgressBar({ steps, onStepClick }: ProgressBarProps) {
  ...
  return (
    <div className="w-full mb-4 sm:mb-6">
      ...
      <div className="sm:hidden flex items-center gap-2 overflow-x-auto scrollbar-hide -mx-3 px-3 relative pb-6">
        {steps.map((step, index) => (
          <div key={step.number} className="flex flex-col items-center flex-shrink-0 min-w-[60px] relative">
            <button
              onClick={() => onStepClick(step.number)}
              className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold text-base mb-1 relative z-10 ${getStepStyles(step)}`}
              aria-label={`Przejdź do kroku ${step.number}: ${step.label}`}
            >
              {step.completed ? (
                <svg
            ...
            <span className={`text-[10px] text-center leading-tight ${getLabelStyles(step)}`}>
              {step.label}
            </span>
            {/* Line between circles */}
            {index < steps.length - 1 && (
