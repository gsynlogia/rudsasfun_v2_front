...
export default function TaborPanel({ tabors, participantNames }: TaborPanelProps) {
  if (tabors.length === 0) {
    return (
  ...
  return (
    <div className="flex flex-col gap-3" data-testid="tabor-panel">
      {tabors.map((t) => <TaborCard key={t.id} tabor={t} participantNames={participantNames} />)}
    </div>
  );
}

function TaborCard({ tabor, participantNames }: { tabor: Tabor; participantNames: Map<number, string> }) {
    ...
    <div className="rounded-lg border border-gray-200" data-testid="tabor-card">
      <button type="button" onClick={() => setExpanded((e) => !e)}
        ...
        <span className="flex gap-1">
          <span title="Edytuj (Nr 28)" className="rounded p-1 text-gray-400 hover:bg-gray-100"><Pencil className="h-4 w-4" /></span>
